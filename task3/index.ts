...
export const findNumber = (arr: any, num: number, place: number): number => {
    ...
    if (!arr.length || (!index && arr[index] !== num)) return -1;
    else if (arr[index] === num) return place + index + 1;
    else if (arr[index] > num)
        return findNumber(arr.slice(0, index), num, place);
    else return findNumber(arr.slice(index), num, place + index);
};
...
(() => {
    ...
    if (
        process.argv[0].includes('entrypoint.js') &&
        (!myArgs.length || isNaN(num))
    ) {
